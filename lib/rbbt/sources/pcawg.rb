...
module PCAWG
  ...
  DATA_DIR.resource = PCAWG

  PROJECT_VAR_DIR = Rbbt.root.var.PCAWG

end

require 'rbbt/sources/pcawg/donor_info'
require 'rbbt/sources/pcawg/expression'
require 'rbbt/sources/pcawg/genotype'
require 'rbbt/sources/pcawg/structural_variants'
...
module PCAWG
  ...
  #PCAWG.claim PCAWG.driver_genes, :proc do
  #  file = PCAWG::DATA_DIR["driver_genes_per_tumor.csv.gz"]
  #  tsv = file.tsv :key_field => "ttype_histology", :header_hash => '', :invert_grep => true, :fix => Proc.new{|l| l.gsub(/([a-z]+)CA(\s)/, '\1Ca\2').gsub(/(ENSG\d+)\.(\d+)/,'\1').gsub(',',';')}, :type => :double, :merge => true, :namespace => PCAWG.organism
  #  tsv.key_field = "histology_abbreviation"
  #  tsv.fields = ['gene', 'Associated Gene Nane', 'Ensembl Gene ID', 'Tumor type', 'Driver source', 'Role', 'Role source']
  #  tsv.slice(tsv.fields[1..-1]).to_s
  #end
  ...
  #  file = DATA_DIR['consensus.20161103.somatic.cna.tar.gz'].find
  #  TmpFile.with_file do |directory|
  #    FileUtils.mkdir_p directory
  #    Misc.in_dir directory do
  #      CMD.cmd("tar xvfz '#{file}'")
  ...
  #      sample2donor = PCAWG.donor_wgs_samples.index :target => PCAWG::DONOR_FIELD
  #      directory.glob("*.txt.gz").each do |file|
  #        sample = File.basename(file).split(".").first
  #        donor = sample2donor[sample]
  #        next if donor.nil?
  #        stream = TSV.traverse Open.open(file), :type => :array, :into => :stream do |line|
